...
{
    // Singleton para que el GameManager pueda llamarlo f�cilmente
    public static UnitSpawner Instance { get; private set; }

    [Header("Prefabs de Unidades")]
    [Tooltip("Arrastra aqu� el prefab del Colono (debe tener el script 'Unit')")]
    ...
    /// <summary>
    /// Punto de entrada principal. El GameManager llamar� a esto despu�s
    ...
    public void SpawnInitialUnits()
    {
        ...
        int landRadius = gridGenerator.boardRadius;// gridGenerator.boardRadius;
        ...
        // Restamos 1 al radio para obtener el �ndice del anillo m�s externo.
        int spawnOffset = landRadius - 1;

        // Si el radio es 1, el offset es 0. Ambos spawns ser�an (0,0).
        ...
        if (humanStartCoords == aiStartCoords && landRadius > 1)
        {
            ...
            Debug.LogWarning("Las coordenadas de inicio eran id�nticas. Moviendo IA a (1,-1).");
        }
    ...
    /// <summary>
    /// Funci�n helper que crea una unidad en el tablero y actualiza todos los sistemas.
    /// </summary>
    /// <param name="owner">El jugador que ser� due�o de la unidad (Humano o IA)</param>
    ...
    private void SpawnUnitAt(Player owner, Vector2Int coords)
    {
        ...
        if (owner == null)
        {
            Debug.LogError("SpawnUnitAt fall�: �El 'owner' es nulo!"); return;
        }
        if (colonoPrefab == null)
        {
            Debug.LogError($"SpawnUnitAt ({owner.playerName}) fall�: �El 'colonoPrefab' no est� asignado en el Inspector!"); return;
        }

        // --- 2. Obtener Celda L�gica ---
        CellData cell = BoardManager.Instance.GetCell(coords);
        if (cell == null)
        {
            Debug.LogError($"Spawn fallido ({owner.playerName}): No se encontr� CellData en las coords {coords}. �Es agua o est� fuera del mapa?");
            return;
        }
