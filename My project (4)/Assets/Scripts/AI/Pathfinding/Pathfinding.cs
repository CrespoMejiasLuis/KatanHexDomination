...
public class Pathfinding : MonoBehaviour
{
    ...
    public List<Vector2Int> FindSmartPath(Vector2Int start, Vector2Int goal, float[,] threatMap)
    {
        ...
        if (threatMap == null)
        {
            // Creamos un mapa de ceros del tama�o correcto
        ...
        Debug.LogWarning("Pathfinding: No se encontr� camino.");
    ...
    private List<Vector2Int> ReconstructPath(Vector2Int current, Vector2Int start)
    {
        ...
        while (current != start && safetyCount < 1000)
        {
            ...
            if (previous.x == -9999)
            {
                Debug.LogError("Error reconstruyendo camino: Eslab�n perdido.");
                break;
            }
    ...
    // Pathfinding específico para combate: ignora amenazas y trata unidades como obstáculos
    public List<Vector2Int> FindPathForCombat(Vector2Int start, Vector2Int goal)
    {
        ...
        Debug.LogWarning("Pathfinding: No se encontró camino para combate.");
